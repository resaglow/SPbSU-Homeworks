...
let balance node = 
    if getBalance node > 1 then
        if getBalance (getLeft node) >= 0 then
            // LL rotation
            match node with
    ...
    else if getBalance node < -1 then 
        if getBalance (getRight node) <= 0 then
            // RR rotation
            match node with
...
type Map<'K, 'V when 'K: comparison and 'V: comparison>(givenTree: AVLTree<'K, 'V>) =
    new (source: seq<'K * 'V>) =
        ...
        let rec add' = function
            ...
            | head :: tail ->
                match head with
                | (k, v) ->
                    curTree := add k v (!curTree)
                    add' tail
        Map<_, _>(!(add' (Seq.toList source)))
    ...
    override this.GetHashCode() = 
        let rec getMapHashCode (listedMap: AVLTree<'K, 'V> list) = 
            match listedMap with
            | [Empty] -> 0
            | Node(_, _, k, v, lh, rh) :: tl -> k.GetHashCode() * ((v.GetHashCode() - (lh * rh).GetHashCode()) % 484317283)
            | _ -> 0
        getMapHashCode (toList givenTree)
