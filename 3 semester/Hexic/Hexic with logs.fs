...
let hexic () =
    ...
    let rec initGame () = 
        ...
        if triples = [] then
            ...
            let toRemove = bfs <| [] <| triples.[0]
            let count = toRemove.Length
            for (i, j) in toRemove do field.[i, j] <- empty

            printfn "All the triples:\n%A" <| triples
            printfn "Cells to remove:\n%A" <| toRemove
            printfn "%A cells to remove" <| count
    ...
    let removeAndShift (sameColored:(int * int) list list) = 
        ...
        let count = toRemove.Length
        if !started then points := !points + countPoints count else ()
        printfn "%A cells to remove" count
        for (i, j) in toRemove do field.[i, j] <- empty
        shiftEmpties ()
    ...
    let rec play all =         
//        printfn "BEFORE ROTATION"
//        printField ()
//        printfn "TRIPLES = %A" <| all        
        match all with
        | triple :: rest -> 
            rotate CW triple
//            printfn "AFTER 1ST ROTATION"
//            printField ()
            ...
            if sameColored = [] then
                ...
                if sameColored2 = [] then
                    ...
                    play rest
                     ...
                     removeAndShift (sameColored2)
                     play <| allTriples ()  
            else 
                 printfn "YES"
                 removeAndShift (sameColored)
                 play <| allTriples ()  
