...
namespace TrainTickets.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public UserStore<ApplicationUser> MyStore { get; private set; }

        public ManageController()
        {
            MyStore = new UserStore<ApplicationUser>(new ApplicationDbContext());
            MyUserManager = new UserManager<ApplicationUser>(MyStore);
        }
        ...
        [HttpPost]
        public async Task<ActionResult> UnBuy()
        {
            var ticketId = Request["ticketId"];

            var boughtTicket = db.Tickets.Find(Convert.ToInt64(ticketId));
            boughtTicket.OwnerLogin = null;
            db.SaveChanges();

            var user = MyUserManager.FindById(User.Identity.GetUserId());
            user.UserMoney += boughtTicket.Price;
            var result = await MyUserManager.UpdateAsync(user);
            MyStore.Context.SaveChanges();

            return RedirectToAction("Index");
        }

        [HttpPost]
        public async Task<ActionResult> SetMoney()
        {
            var gotMoney = Convert.ToDecimal(Request["UserMoney"]);

            var user = MyUserManager.FindById(User.Identity.GetUserId());
            user.UserMoney = gotMoney;
            var result = await MyUserManager.UpdateAsync(user);
            MyStore.Context.SaveChanges();
